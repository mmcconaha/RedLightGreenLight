...
export async function GET(req: Request) {
  ...
  if (noteRows.length > 0) {
    // Private to this member only — never read by the organizer view or
    // anyone else. See the RLS policies on private_notes.
  ...
  // Debug: show exactly what UTC boundaries our own code computed for each
  // block on the date of the first event, so we can compare directly
  // against that event's raw start/end printed above.
  let boundaryDebug = "";
  if (events.length > 0) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (params2.get("synced") === "1") {
        ...
        const boundaries = params2.get("boundaries");
        ...
        if (boundaries) msg += ` || Our boundaries: ${decodeURIComponent(boundaries)}`;
        setJustSynced(msg);
...
RESPOND_EOF
echo "Verifying..."
grep -c "boundaries" "app/api/auth/google/callback/route.ts"
echo "Done."
