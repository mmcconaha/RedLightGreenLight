#!/bin/bash
set -e
echo "Writing OAuth callback route (with event count debug)..."
...
export async function GET(req: Request) {
  ...
  return NextResponse.redirect(
    new URL(`/session/${sessionId}/respond?synced=1&rows=${rows.length}&events=${events.length}${noteErrorParam}`, url.origin)
  );
}
CALLBACK_EOF
...
RESPOND_EOF
echo "Verifying..."
grep -c "events.length" "app/api/auth/google/callback/route.ts"
echo "Done."
