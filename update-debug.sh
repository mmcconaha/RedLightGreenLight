...
export async function fetchFreeBusy(
  ...
  const json = await res.json();
  if (json.error) {
    throw new Error(`freebusy error: ${json.error.message || JSON.stringify(json.error)}`);
  }
  const cal = json?.calendars?.primary;
  if (cal?.errors?.length) {
    throw new Error(`calendar error: ${JSON.stringify(cal.errors)}`);
  }
  return cal?.busy ?? [];
}
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (params2.get("synced") === "1") {
        const busycount = params2.get("busycount");
        const rows = params2.get("rows");
        setJustSynced(`Synced — found ${busycount ?? "?"} busy blocks on your calendar, marked ${rows ?? "?"} time slots red below.`);
      }
